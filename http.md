...
**HTTP** - это один из основных протоколов интернета, этот протокол описывает взаимодействие между двумя компьютерами (клиентом и сервером), построенное на базе сообщений, называемых запрос **(Request)** и ответ **(Response)**. Каждое сообщение состоит из трех частей: _стартовая строка, заголовки и тело_. При этом обязательной является только стартовая строка.
...
**URI** - расшифровывается, как _Uniform Resource Identifier_ — единообразный идентификатор ресурса
...
**Сейчас по протоколу HTTP можно создавать** 
* посты
* редактировать профиль
* удалять сообщения и многое другое
...
_Есть небольшая проблема с применением REST на практике. Проблема эта называется HTML_
